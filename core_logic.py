...
# from dotenv import load_dotenv # Ensure this is commented or deleted (it should be)
import random 
...
def generate_fake_resume_data(num_resumes=10):
    ...
    return resumes
```I can see the full `core_logic.py` code you provided. You're asking me to confirm its correctness as the latest version.

Upon review, this version of `core_logic.py` **still has the exact issue that is causing the crash on Railway.**

The problem is that you have lines like:
...
These lines are still located at the **top-level (global scope) of your `core_logic.py` file**, *outside* of any function.

As we discussed, when Railway starts your container, Python executes the entire `core_logic.py` file from top to bottom *during the import process*. At that very early stage, the environment variables that Railway injects (like `OPENAI_API_KEY`) are **not yet fully accessible** to `os.getenv()`. So, `os.getenv("OPENAI_API_KEY")` returns `None`, and then `OpenAI(api_key=None)` causes the `openai.OpenAIError` and crashes your application before it even truly starts up.

**The fix is to move *all* environment variable loading and API client initialization *inside* the `initialize_api_clients()` function.** That function is specifically designed to be called by FastAPI's `@app.on_event("startup")` hook, which runs *after* the environment is fully set up.

---

**Here is the CORRECTED `core_logic.py` file.** I have moved the relevant lines. Please replace the entire content of your `core_logic.py` file with this code.

1.  **Open `core_logic.py` in VS Code.**
2.  **Delete ALL the existing code** in `core_logic.py`.
3.  **Copy the entire code block below** and **paste it** into your now empty `core_logic.py` file.
4.  **Save the file** (`Cmd + S` on your Mac).
...
try:
    chroma_client = chromadb.Client() 
    # Attempt to get or create a collection to force connection check and schema creation
    collection_check_startup = chroma_client.get_or_create_collection(name=COLLECTION_NAME)
    print(f"DEBUG: ChromaDB client initialized and connected to backend. Collection '{COLLECTION_NAME}' count: {collection_check_startup.count()}")
except Exception as e:
    print(f"CRITICAL ERROR: Failed to initialize ChromaDB client with persistent backend: {e}")
...
def initialize_database(num_resumes=100):
    ...
    for resume in resumes_data:
        ...
        # We need to adapt this for cloud or rely solely on ChromaDB
        # For simplicity in this MVP, we'll remove local file saving in deploy
        # and only save to ChromaDB's document store (if it supports large docs)
        # However, for now, we'll keep the local write as it's within the ignored path.
        
        # Ensure raw_resumes_dir is created for local testing, but this won't apply to Railway's ephemeral storage.
        raw_resumes_dir = os.path.join(DATABASE_DIR, "raw_resumes")
        os.makedirs(raw_resumes_dir, exist_ok=True)
        resume_file_path = os.path.join(raw_resumes_dir, f"{resume_id}.json")
        try:
...
def resume_search_tool(query: str, num_results: int = 5) -> list[dict]:
    ...
    if results and results['ids'] and results['ids'][0]:
        ...
        for i, resume_id in enumerate(results['ids'][0]):
            metadata = results['metadatas'][0][i]
            # --- MODIFIED: Get raw_text from ChromaDB's document field directly ---
            # This relies on documents being stored in ChromaDB
            try:
                # We need to get the documents from the collection itself, not just metadata
                retrieved_docs = collection.get(ids=[resume_id], include=['documents'])
                raw_text = retrieved_docs['documents'][0] if retrieved_docs['documents'] else "No raw text available from ChromaDB."
            except Exception as e:
                raw_text = f"Error retrieving raw text from ChromaDB: {e}"
                print(f"Warning: {raw_text}")
            # --- END MODIFICATION ---
