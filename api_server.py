...
VALID_API_KEYS = {"a885598e-1a1b-463b-9c23-acc4e4275330", "05f84d16-3908-4da3-8804-5c455d4acc1c"}
...
async def search_candidates(request: Request, search_request: SearchRequest, api_key: str = Depends(get_api_key)):
    try:
        result_data = core_logic.perform_claude_search_with_tool(
            user_query=search_request.query, num_profiles_to_retrieve=search_request.num_results,
            source=search_request.source, folder_ids=search_request.google_drive_folder_ids,
            user_id=api_key, token=search_request.google_auth_token
        )
